...
const Burgerlar = () => {
  ...
  const getData = async () => {
    try {
      const respons = await axios.get(
        `https://ce1a1e26cf80d56c.mokky.dev/cardsburgerlar/${params.id}`
      );
  ...
  return (
    <div>
    <div className="w-full container px-14 py-[60px] border mx-auto mb-[30px] gap-10 m-[40px]">
    {errMasg.length > 0 ? (
      <h1>{errMasg}</h1>
    ) : (
      <div className="flex w-full justify-evenly gap-[50px] items-center">
       ...
       <h1 className="font-bold text-[40px]">{data.title}</h1>
        <h1 className="w-[400px] text-[20px] text-[#6E7C87]">{data.text}</h1>
        <button onClick={() => onAdd(data)} className="w-[200px] h-[40px] bg-[#309B42] text-white flex justify-evenly align-center items-center mt-[40px]"> Karzinkaga
